...
**Experimental unit**:
The unique combination of a study's grouping columns that identifies one observation for the mean relative improvement (and SD / IQR descriptors) inside a by-precision aggregation (for example a model, or a model–task pair). Not necessarily a DL model alone, and not necessarily an independent draw when tasks or datasets nest inside a model family.
...
**Cluster unit**:
The nesting factor treated as the independent unit for effective sample size, sample-size discount, and interval estimation when experimental units share a model family. It is the experimental-unit key after dropping evaluation-context columns (`Dataset`, `dataset`, `task`) only — Device, compute unit, frequency, library, and filter multiplier stay. Distinct from the experimental unit used for the mean and SD (variability discount) / IQR (descriptive).
...
Analysis that tests whether primary synthesis conclusions are robust when main synthesis assumptions change — either pooling / belief-assignment assumptions or the RI→effect-intensity encoding. The published lead dependence check holds the included study set and extracted intensities fixed and recombines evidence under the undiscounted mass-preserving belief split ($1-(1-B)^{1/N}$ per evidence model), deliberately omitting \(\alpha_n\) and \(\alpha_v\) so the table isolates repeated commitment of study belief rather than mixing that allocation with discounting (ADR 0010). Leave-one-study-out recombination of the main discounted pooling and the equitable split ($B/N$) are supplementary appendix stress tests. A further appendix stress test varies the main discount parameters one at a time without replacing the published forest plots: \(n_0\in\{2,n_0^{Q3},6\}\), \(k\in\{0.05,0.1,0.5\}\), and cutoff \(\in\{3,4,8\}\), reporting aggregated direction/intensity and accuracy \(B'\). Another appendix stress test perturbs RI→intensity cut-points one at a time (functional-suitability and resource/performance scales), holding the study set and discounted support masses fixed and without replacing the published forest plots; aggregated intensity and belief measure may change when remapped focal elements are recombined (ADR 0014). Distinct from subgroup analysis, which restricts the moderator triple while holding the study set fixed.
_Avoid_: Subgroup analysis (when the cut is which studies are in, not which cell); reusing corpus-wide Aggregated rows as if they were sensitivity-sample-specific; study-set exclusion by theoretical-structure count (retired TS $\le 5$ analysis); publishing a discounted mass-preserving root as the lead dependence check; calling cut-point stress tests a separate analysis outside sensitivity; treating intensity remapping as a pooling assumption
...
_Avoid_: Min-belief downgrade of theory arrows; replacing Results primary intensity with the dependence check; treating belief drops alone as intensity changes; feeding cut-point or discount-parameter appendix stress tests into theory arrows
...
The reliability factor \(\alpha_v=e^{-k\,V_2}\) with \(k=0.1\) (and \(\alpha_v=1\) when cluster \(n_{\mathrm{eff}}\le 4\)) applied to study belief when experimental-unit relative improvements inside one by-precision aggregation are heterogeneous. \(V_2=\sigma/\sqrt{\sigma^2+\mu^2}\) is Kvålseth’s second-order coefficient of variation on those experimental-unit relative improvements (signed RI% allowed; well-defined when \(\mu\approx 0\)). Distinct from by-precision aggregation itself, which is the grouping, not the discount.
_Avoid_: \(\mathrm{IQR}/(\mu+\varepsilon)\) as the main relative-dispersion argument; floored \(\max(|\mu|,\delta)\); Heath PV / CQV / RCVQ as the main argument without a positive embedding; treating this discount as a second meaning of aggregation

**Second-order coefficient of variation**:
Kvålseth’s bounded relative-dispersion measure \(V_2=\sigma/\sqrt{\sigma^2+\mu^2}\in[0,1]\) on a real-valued sample (here: experimental-unit relative improvements). It replaces Pearson’s \(V=\sigma/|\mu|\) when values may be signed or the mean may be near zero.
_Avoid_: Pearson CV; Heath proportional variability (non-negative only); CQV on signed RI%
