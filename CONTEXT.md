...
The component-wise numeric-format assignment of a quantized run. The canonical form uses tokens `<component>-<format>` joined by `", "`, with components in fixed order `w`, then `a`, then `b`, then any others alphabetically, and omits unmentioned ones — so `w-int8` means weights-only, not an implied `a-fp32`. Distinct assignments must not be collapsed: `w-int8, a-int4` is not the same as `w-int8, a-int8`. A bare label such as `int8` is an input alias for `w-int8, a-int8` only when equal weight and activation formats are actually known; it must not invent components for weight-only (or otherwise partial) studies. Compact forms (`wa-int8`, `w8a8`) and method-prefixed forms (`qat-w8a8`) are input aliases only, not canonical.
...
**Numeric format**:
The atomic format label used inside a precision configuration or full precision configuration. Distinct families are kept separate and are not rewritten into each other: `intN`, `fpN`, `q0.N`, `qM.F` (fixed-point integer.fractional bits), `logN`, and `mixed`. Typos normalize within a family only (e.g. `q0,32` → `q0.32`); `q0.8` is not the same as `q8.0`.
_Avoid_: Bit width (when the family/spelling matters), precision configuration (when a single atom is meant)
